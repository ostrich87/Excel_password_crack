...
def help_file() : # 메세지박스 띄우기
    messagebox.showinfo("HELP", "\nFind Excel(Security) password Version 1.0 (Trial Version)\n\n"+
                                ...
                                "  For 10,000 cases, it takes about 10 minutes\n" +
                                "  (The time required may vary depending on the computer\n" +
                                "   specifications)\n\n" +
                                " [Notic] CLose the MS-Excel Program and run it.\n" +
                                "                                                  Date : 12/28/2022\n" +
                                "                                                  jyh4ever@gmail.com\n\n\n\n") 
...
def file_find_fun_tab1():
    ...
    window.filename = filedialog.askopenfilename(initialdir = cur_dir,title = "choose your file",filetypes = (("Excel files","*.xlsx"),("Excel files","*.xls")))
...
def file_find_fun_tab2():
    ...
    window.filename = filedialog.askopenfilename(initialdir = cur_dir,
                      title = "choose your file",filetypes = (("Excel files","*.xlsx"),("Excel files","*.xls")))
    dis_file = os.path.basename(window.filename)  # 파일명만 추출
...
def ok_function_tab2(): 
    ...
    if (q_result == True) :  
        ...
        if (find_name_tab2 == None or find_name_tab2 == '' ) :                                 # EXCEL 파일 체크
            result_value = 1 
            error_file_exists(2)
        ...
        if (find_type_var_tab2.get() == 1) and (result_value == 0) :   
            ...
            if (file_password_check1(2) == True) and (file_password_check2(2) == True) :   # 암호가 걸렸는지 체크
                ...
                for text_i in text_array:
                    ...
                    progress_percentage = percentage_fun(chars_case_num_count,chars_case_num_total)  # 소숫점 이하 3자리까지 백분율 표시
                    Progress_display(2,progress_percentage) # 진행율을 보여줌
                    chars_case_num_count = chars_case_num_count + 1
                    p_var = DoubleVar()
                    progressbar = tkinter.ttk.Progressbar(tab2, maximum=chars_case_num_total,mode="determinate", length=350, variable = p_var )
                    progressbar.pack()
                    progressbar.place(x=150,y=305)
...
def error_display_length(tab_value) :  # tab 1, 3
    if (tab_value == 1) :   
        lb_hi = Label(tab1, text="Length ERROR !!", fg='red', width=20, height=1,padx=5, pady=5,justify=[RIGHT])
        lb_hi.place(x=500,y=80)
    elif (tab_value == 3) :
        lb_hi = Label(tab3, text="Length ERROR !!", fg='red', width=20, height=1,padx=5, pady=5,justify=[RIGHT])
        lb_hi.place(x=500,y=80)
    ...
    for length in range(r_find_length_min,r_find_length_max+1) :   
        ...
        for password in allpass:
            ...
            if (chars_case_num_count % display_cycle_value == 0 or chars_case_num_count == 1) :
                ...
                Progress_display(tab_value,progress_percentage) # 진행율을 보여줌.  표시주기 변경
                # progress bar
                if (tab_value == 1): 
                    p_var = DoubleVar()
...
# tab1 GUI ================================================================================================
# Excel destination  입력
lb_hi = Label(tab1, text=" Excel Destination", bg='gray93', fg='gray1',width=15, height=1,padx=5, pady=5, justify=[RIGHT])
...
# tab2 GUI ================================================================================================
# Excel destination  입력
lb_hi = Label(tab2, text=" Excel Destination", bg='gray93', fg='gray1',width=15, height=1,padx=5, pady=5, justify=[RIGHT])
lb_hi.place(x=30,y=40)
...
# tab3 GUI ================================================================================================
# Excel destination  입력
lb_hi = Label(tab3, text=" Excel Destination", bg='gray93', fg='gray1',width=15, height=1,padx=5, pady=5, justify=[RIGHT])
lb_hi.place(x=30,y=40)
